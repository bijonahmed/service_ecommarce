<template>
    <title>Edit Global Wallet Address</title>
    <div>
...
const saveData = () => {
    ...
    axios
        .post("/category/editglobalwalletAddress", formData, {
            headers: {
...
const checkRow = () => {
        ...
        insertdata.id = id;
         axios.get(`/category/globalAddressrow/${id}`).then(response => {
            console.log("=====" + response.data.data.lock_unlock);
        insertdata.id = response.data.data.id;
        insertdata.name = response.data.data.name;
        insertdata.lock_unlock = response.data.data.lock_unlock;
