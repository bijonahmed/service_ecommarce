<template>
    <title>Mining Category List</title>
    <div>
                        ...
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                ...
                                <li class="breadcrumb-item active">
                                    <LazyNuxtLink to="/setting/mining-category-add">New Mining Category</LazyNuxtLink>
                                </li>
                            </ol>
                        </div>
                    </div>
                                        ...
                                        <table class="table w-100 table-wrapper">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="text-center">Duration in hours</th>
                                                    <th class="text-center">Minutes</th>
                                                    <th class="text-center">Seconds</th>
                                                    <th class="text-center">Minining Amount Per Second</th>
                                            ...
                                            <tbody>
                                                <tr v-for="item in productdata" :key="item.id">
                                                    ...
                                                    <td class="text-center">{{ item.duration_in_hour }}</td>
                                                    <td class="text-center">{{ item.minute }}</td>
                                                    <td class="text-center">{{ item.second }}</td>
                                                    <td class="text-center">{{ item.minining_amount_per_secnd }}</td>
...
const fetchData = async (page) => {
    try {
        loading.value = true;
        const response = await axios.get(`/category/allMiningCategoryes`, {
            params: {
                page: page,
                pageSize: pageSize,
...
const edit = (id) => {
    router.push({
        path: '/setting/mining-category-edit',
        query: {
            parameter: id
        }
    });
