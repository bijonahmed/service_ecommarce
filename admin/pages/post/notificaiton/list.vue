                        ...
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                ...
                                <li class="breadcrumb-item active">
                                    <LazyNuxtLink to="/post/notificaiton/add">New</LazyNuxtLink>
                                </li>
                            </ol>
                        </div>
                    </div>
                                        ...
                                        <table class="table w-100 table-wrapper">
                                            ...
                                            <tbody>
                                                <tr v-for="item in productdata" :key="item.id">
                                                    ...
                                                    <td class="text-left">{{ item.messages }}</td>
...
const fetchData = async (page) => {
    try {
        loading.value = true;
        const response = await axios.get(`/post/allNotificationMsg`, {
            params: {
                page: page,
                pageSize: pageSize,
...
const deleteProduct = (id) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            // User confirmed, now delete the product
            console.log('Deleting item with id:', id);

            // Perform your delete logic here, e.g., make an API request to delete
            axios.delete(`/post/notifidelete/${id}`)
                .then(response => {
