<template>
    <div class="referral-link-container">
        <h2 class="heading">Your Referral Link</h2>
...
const chkUserrow = async () => {
    try {
        ...
        invite_code.value = response.data.invite_code // Set the invite code
    } catch (error) {
        console.error("Error fetching user data:", error)
    }
}

// Update referralLink whenever invite_code changes
watchEffect(() => {
    const currentDomain = window.location.origin
    referralLink.value = `${currentDomain}${router.resolve({ path: '/invite-code' }).href}?code=${invite_code.value}`
})
